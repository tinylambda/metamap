...
class Command(MetaCommand):
    help = (
        'Create basic nginx configuration file for this project, '
    ...
    def prepare_configuration_files(self):
        ...
        websocket_conf_filename = getattr(
            settings, 'NGINX_WEBSOCKET_CONF_FILENAME', None
        )
        ...
        if nginx_context['NGINX_HTTP_SERVER_NAME']:
            with open(http_conf_filename, 'w') as f:
                f.write(
                    render_to_string(
                        'conf/nginx/default-http.conf', context=nginx_context
                    )
                )
                self.stdout.write(
                    self.style.SUCCESS(
                        f'written Nginx http conf to {http_conf_filename}'
                    )
                )
        else:
            self.stderr.write(
        ...
        if nginx_context['NGINX_WEBSOCKET_SERVER_NAME']:
            with open(websocket_conf_filename, 'w') as f:
                f.write(
                    render_to_string(
                        'conf/nginx/default-websocket.conf', context=nginx_context
                    )
                )
                self.stdout.write(
                    self.style.SUCCESS(
                        f'written Nginx websocket conf to {websocket_conf_filename}'
                    )
                )
        else:
            self.stderr.write(
